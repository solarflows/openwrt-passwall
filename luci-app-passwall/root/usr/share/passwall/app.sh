...
stop() {
	...
	busybox pgrep -af "${CONFIG}/" | awk -v CFG="${CONFIG}" '! /app\.sh|subscribe\.lua|rule_update\.lua|tasks\.sh|server_app\.lua|ujail/ && $0 !~ ("init\\.d/" CFG) {print $1}' | xargs -r kill -9 >/dev/null 2>&1
